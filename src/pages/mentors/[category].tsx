    ...
    useEffect(() => {
        if (typeof category === 'string') {
            const getMentorList = async () => {
                    ...
                    const selectedCategoryLabel = selectedCategory ? selectedCategory.label : 'Category Not Found';
                    ...
                    try {
                        ...
                        const filteredMentorData = mentorData.filter(mentor => mentor.Category.includes(selectedCategoryLabel));
                        // @ts-ignore
                        setAllMentorList(filteredMentorData);
            ...
            <div className="bg-[#F1F4F9] flex flex-col w-[100vw] items-start">
                <div className="mx-10 md:mx-20 py-10 w-[85vw] md:w-[90vw]">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 items-start gap-x-4 w-full pt-10">
                        {allMentorList.length > 0 ? allMentorList.map((mentor: any) => (
                            <div
                                key={mentor.uid}
                            ...
                            >
                                ...
                                <div  className="flex flex-row justify-between items-start w-full">
                                    <div className="flex flex-col justify-start w-full">
                                        ...
                                        <div className="flex flex-row gap-2">
                                            ...
                                            <p className="text-[#6C6C6C] font-semibold">{mentor.Description.slice(0, 35)}</p>
                                            {mentor.Description.length > 35 ? <p className="text-[#6C6C6C] font-semibold">...</p> : null}
                                        </div>
                                        <div className="flex flex-row gap-x-1 items-center justify-start w-full -translate-x-1.5">
                                            ...
                                            <p className="text-[#FFAB00] font-semibold">{mentor["Start Price Range"]+"-"+mentor["End Price Range"]}</p>
                                        </div>
                                    </div>
                                </div>
                                <div className="flex flex-row justify-between items-center text-[#6C6C6C] bg-[#FAFAFA] w-full px-5 rounded-2xl">
                                    <div>
                                        <p>Experience</p>
                                        <p className="text-[#545454] font-bold">{mentor["Experience in Years"]} years</p>
                                    </div>
                                    <div>
                        ...
                        )) : <p className="h-[30vh] text-xl text-black items-center text-center opacity-80 w-[100vw]">
